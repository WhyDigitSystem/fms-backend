...
public interface DocumentTypesMappingRepo extends JpaRepository<DocumentTypesMappingVO, Long>{
	...
	@Query(nativeQuery = true, value = "select screencode,screenname,doccode,?5,?3,?1,?2,concat(?2,?5,doccode)prefixField from documenttypes where orgid=?4 and concat(?1,?2,?3,screencode,doccode,orgid) not in(    \r\n"
			+ " select concat(branch,branchcode,finyear,screencode,doccode,orgid) from documenttypesmappingdetails where orgid=?4)")
Set<Object[]> findAllDocumentTypesMappingDetailsByDocumentType(String branch, String branchCode, String finYr,Long orgId,String finyrId);
